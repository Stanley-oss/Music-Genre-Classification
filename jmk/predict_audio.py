...
DEFAULT_CKPT = Path(r"D:\code\codepy\dl\emf_v1_out\best_emf_v1.pt")
DEFAULT_OUT_DIR = Path(r"D:\code\codepy\dl\infer_out")
...
class EMFv1(nn.Module):
    def __init__(self, num_classes: int = NUM_CLASSES, emb_dim: int = EMB_DIM) -> None:
        super().__init__()
...
def load_checkpoint(ckpt_path: Path, device: torch.device):
    ...
    model = EMFv1(num_classes=len(label_to_id)).to(device)
