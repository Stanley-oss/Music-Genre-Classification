...
SEG_DIR = Path(r"D:\code\codepy\dl\preprocessed\seg_3s_base")
MEL_DIR = Path(r"D:\code\codepy\dl\mel_cache\lm3_base_v1")
OUT_DIR = Path(r"D:\code\codepy\dl\emf_v1_out")
TRAIN_ROOT_DIR = Path(r"D:\code\codepy\dl\emf_train_runs")
ABLA_ROOT_DIR = Path(r"D:\code\codepy\dl\emf_ablation_runs")
DEFAULT_CKPT_DIR = Path(r"D:\code\codepy\dl\emf_v1_out")

# Minimal sweep knobs. Default keeps original v1 architecture.
TEMPORAL_KIND = "conformer"  # choices: conformer, mlp
USE_DENOISING = True
...
class EMFv1(nn.Module):
    ...
    def __init__(
        ...
        if temporal_kind == "conformer":
            ...
            self.temporal_refiner = TemporalMLPBlock(channels=160, dropout=0.15)
        else:
            raise ValueError(f"Unknown temporal_kind: {temporal_kind}")
        self.pool = TimeAttentionPool(channels=160)
        self.to_emb = nn.Sequential(
            nn.LayerNorm(160),
            nn.Linear(160, emb_dim),
            nn.GELU(),
            nn.Dropout(0.20),
...
def structure_to_knobs(name: str) -> Tuple[str, bool, str]:
    ...
    - mlp: replace TemporalConformerBlock with simple TemporalMLPBlock, keep denoising
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run MusicFlowNet ablation experiments only. "
            "It compares the full model with two planned ablations: "
            "Temporal Context Block -> MLP, and removing embedding denoising."
        )
    )
    parser.add_argument(
        "--seeds",
        type=str,
        default="0,1,2",
        help="Comma-separated seeds. Default is 0,1,2 for a fair small ablation run.",
    )
    parser.add_argument(
        "--run_root",
        type=str,
        default=str(ABLA_ROOT_DIR),
        help="Root folder for ablation outputs.",
    )
    parser.add_argument(
        "--build_mel",
        type=str_to_bool,
        default=True,
        help="Build or check log-mel cache before experiments. Existing cache files are skipped.",
    )
    parser.add_argument(
        "--export_explain",
        type=str_to_bool,
        default=False,
        help="Usually False for ablation runs to save time.",
    )
    parser.add_argument(
        "--force_cuda",
        type=str_to_bool,
...
def main() -> None:
    ...
    # Keep the original planned ablations only:
    # full  = TemporalConformerBlock + embedding denoising
    # mlp   = replace TemporalConformerBlock with a simple MLP, keep denoising
    # nodn  = keep TemporalConformerBlock, remove embedding denoising
    structures = ["full", "mlp", "nodn"]

    rows: List[Dict[str, object]] = []
    summary_path = run_root / "ablation_summary.csv"
    for structure_name in structures:
        temporal_kind, use_denoising, structure_tag = structure_to_knobs(structure_name)
        for seed in seeds:
            out_dir = run_root / f"v1_{structure_tag}_s{seed}"
    ...
    print("\n[done] ablation summary:")
    for row in rows:
        print(
            f"{row['structure']} seed={row['seed']} "
            f"seg_acc={row.get('segment_acc')} song_acc={row.get('song_acc')} "
            f"| {row['out_dir']}"
        )
