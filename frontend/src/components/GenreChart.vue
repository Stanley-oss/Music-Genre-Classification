...
const tooltipFormatter = function (params) {
  ...
  const hoveredTime = Number(params[0].value[0]);
  
  // Find closest history item robustly
  let histItem = props.history[0];
  let minDiff = Math.abs(histItem.t - hoveredTime);
  for (let i = 1; i < props.history.length; i++) {
    const diff = Math.abs(props.history[i].t - hoveredTime);
    if (diff < minDiff) {
      minDiff = diff;
      histItem = props.history[i];
    }
  }
  ...
  let timeStr = histItem.t.toFixed(1);
...
const option = ref({
  ...
  series: props.genres.map((g, i) => ({
    name: g, type: 'line', smooth: false, showSymbol: true, symbolSize: 6,
    connectNulls: false, lineStyle: { width: 2 },
    itemStyle: { color: palette[i % palette.length] }, data: []
  }))
})

watch(() => [props.history, props.time, props.duration, zoomStart.value, zoomEnd.value], () => {
  const xMax = props.duration && props.duration > 0 ? Math.ceil(props.duration) : Math.max(10, props.time + 2)
  ...
  option.value.series = props.genres.map((g, i) => ({
    name: g, type: 'line', smooth: false, showSymbol: true, symbolSize: 6,
    connectNulls: false, lineStyle: { width: 2 },
