...
stats = json.load(open(os.path.join(ablation_code_dir, 'mel_cache', 'lm3_base_v1', 'stats_train.json')))
...
seg_dir = os.path.join(current_dir, '..', 'Ablation', 'code', 'preprocessed', 'seg_3s_base', 'test')
test_files = []
for genre in GENRES:
...
print(f"logmel (before norm) shape={logmel.shape}, min={logmel.min():.4f}, max={logmel.max():.4f}, mean={logmel.mean():.4f}")
print(f"logmel_norm shape={logmel_norm.shape}, min={logmel_norm.min():.4f}, max={logmel_norm.max():.4f}, mean={logmel_norm.mean():.4f}")
...
for tk in ['cnn', 'resnet', 'lstm']:
    ckpt_path = os.path.join(ablation_code_dir, 'emf_train_runs_seed2025_backbones',
                             f'{tk}_60ep_s2025', 'best_emf_v1.pt')
...
mel_module = WaveformToMelExact(sr=SR, n_fft=N_FFT, hop_length=HOP, n_mels=N_MELS, frames=96)
mel_module.eval()
...
print(f"Export mel min={mel_from_export.min():.4f}, max={mel_from_export.max():.4f}, mean={mel_from_export.mean():.4f}")

# Compare with librosa pipeline on the same audio segment
y_segment = audio_patch
mel_seg = librosa.feature.melspectrogram(y=y_segment, sr=SR, n_fft=N_FFT, hop_length=HOP,
                                          n_mels=N_MELS, fmin=FMIN, fmax=FMAX, power=2.0)
...
for tk in ['cnn', 'resnet', 'lstm']:
    ...
    onnx_probs = sess.run([output_name], {input_name: audio_patch.reshape(1, -1).astype(np.float32)})[0][0]
