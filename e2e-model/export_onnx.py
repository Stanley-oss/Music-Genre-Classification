...
class WaveformToMelExact(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mel_power = torch.matmul(power.transpose(1, 2), self.mel_filterbank).transpose(
            1, 2
        )
...
class E2E_Unified(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.waveform_to_mel = WaveformToMelExact(
            sr=sr, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels, frames=frames
        )
        self.emfv1 = EMFv1(
            num_classes=10, temporal_kind=temporal_kind, use_denoising=True
        )
...
def export_model(temporal_kind, out_path):
    ...
    if os.path.exists(ckpt_path):
        ...
        print(
            f"[WARNING] Checkpoint not found at {ckpt_path}, exporting with random weights."
        )
